...
from pykiwoom.error_handler import *
from pandas import DataFrame

from utils import clean_duplicate_2d
...
class Kiwoom(QAxWidget):
    ...
    def on_receive_tr_data(self, screen_no, request_name, tr_code, record_name, inquiry, unused0, unused1, unused2,
        ...
        if request_name == "주식일봉차트조회요청":
            ...
            if inquiry == "0" or self.inquiry == 0:
                col_name = ['일자', '시가', '고가', '저가', '종가', '전일비', '등락률', '거래량', '금액(백만)', '신용비', '개인',
                            '기관', '외인수량', '외국계', '프로그램', '외인비', '강도', '외인보유', '외인비중', '외인순매수',
                            '기관순매수', '개인순매수', '신용잔고율']
        ...
        elif request_name == "실시간미체결요청":
            cnt = self.get_repeat_cnt(tr_code, request_name)
            not_concluded = []
            key_list = ["주문구분", "주문번호", "종목명", "종목코드", "주문수량", "주문가격", "현재가"]

            for i in range(cnt):
                col_values = []
                for key in key_list:
                    col_values.append(self.comm_get_data(tr_code, request_name, i, key))
                not_concluded.append(col_values)
            self.opt10075 = not_concluded

        elif request_name == '계좌평가잔고내역요청':
            # 계좌 평가 정보
            account_evaluation = []
            key_list = ["총매입금액", "총평가금액", "총평가손익금액", "총수익률(%)", "추정예탁자산"]
            self.data_opw00018['stocks'] = clean_duplicate_2d(self.data_opw00018['stocks'])
            ...
            # 보유 종목 정보
            cnt = self.get_repeat_cnt(tr_code, request_name)
            key_list = ["종목명", "종목번호", "보유수량", "매입가", "현재가", "평가손익", "수익률(%)", "종목번호"]
    ...
    def on_receive_chejan_data(self, gubun, item_cnt, fid_list):
        ...
        print("on_receive_chejan_data 실행 구분 : ", gubun)
        self.refresh = True
        ...
        # callback 변수 만들어 True일 시 GetChejanData 이후 refresh
    ...
    @critical_error_decorator((KiwoomConnectError, ParameterTypeError, ParameterValueError))
    ...
    def comm_rq_data(self, request_name, tr_code, inquiry, screen_no):
        ...
        return_code = self.dynamicCall("CommRqData(QString, QString, int, QString)",
                                       request_name, tr_code, inquiry, screen_no)
    ...
    def comm_get_data(self, code, field_name, index, item_name):
        """
        데이터 획득 메서드
    ...
    def commKwRqData(self, codes, inquiry, codeCount, requestName, screenNo, typeFlag=0):
        ...
        량, 강도, 전일거래량대비, 매도호가, 매수호가, 매도1~5차호가, 매수1~5차호가,
        상한가, 하한가, 시가, 고가, 저가, 종가, 시간, 예상가, 예상량, 자본금,
    ...
    def receive_real_condition(self, code, event, condition_name, condition_index):
        ...
        :param code: string - 종목코드
        :param event: string - 이벤트("I": 종목편입, "D": 종목이탈)
        :param condition_name: string - 조건식 이름
    ...
    def send_order(self, request_name, screen_no, account_no, order_type, code, qty, price, bid_type, origin_order_no):
        """
        주식 주문 메서드
        ...
        :param bid_type: string - 거래구분(00: 지정가, 03: 시장가, 05: 조건부지정가, 06: 최유리지정가, 그외에는 api 문서참조)
        ...
        return_code = self.dynamicCall("SendOrder(QString, QString, QString, int, QString, int, int, QString, QString)",
                                       [request_name, screen_no, account_no, order_type, code, qty, price, bid_type,
                                        origin_order_no])
    ...
    def get_master_code_name(self, code):
        ...
        cmd = "GetMasterCodeName(QString)"
        name = self.dynamicCall(cmd, [code])
        return name
